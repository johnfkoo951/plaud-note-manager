    ...
    var body: some View {
        PlaudWebLoginView(
            ...
            onStatus: { status in
                ...
                } else if status.hasPrefix("Plaud silent recovery failed:")
                    || status.hasPrefix("Plaud Web failed to load:") {
                    store.deferAuthRecovery(status)
                }
            },
            recoveryMode: true,
    ...
    private func handleCapture(_ capture: PlaudWebAuthCapture) {
        ...
        Task {
            ...
            if ok {
                ...
                store.deferAuthRecovery(detail)
            }
        }
    }
}
