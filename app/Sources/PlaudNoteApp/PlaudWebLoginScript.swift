...
(() => {
  ...
  const captureGeneration = window.__plaudNativeCaptureGeneration;
  const isPlaudAPI = (rawURL) => {
    try {
      const url = new URL(String(rawURL || ""), window.location.href);
      return url.protocol === "https:" && !url.username && !url.password
        && (!url.port || url.port === "443")
        && /^api(?:-[a-z0-9-]+)?\.plaud\.ai$/i.test(url.hostname);
    } catch (_) {
      return false;
    }
  };
  const post = (url, headers) => {
    try {
      ...
      if (!isPlaudAPI(rawURL)
          || window.__plaudNativeCaptureGeneration !== captureGeneration) { return; }
      window.webkit.messageHandlers.plaudAuthCapture.postMessage({
        kind: "capture", generation: captureGeneration,
        url: rawURL,
        headers: headers || {}
      });
    } catch (_) {}
...
(async () => {
  ...
  const captureGeneration = window.__plaudNativeCaptureGeneration;
  const currentGeneration = () => window.__plaudNativeCaptureGeneration === captureGeneration;
  const abortController = new AbortController();
  window.__plaudRecoveryAbort = abortController;
  const status = (state, detail) => {
    if (!currentGeneration()) return;
    try { bridge.postMessage({ kind: "recoveryStatus", generation: captureGeneration,
  ...
  const normalizeDomain = (raw) => {
    const value = String(raw || "https://api.plaud.ai").trim();
    const url = new URL(value.includes("://") ? value : `https://${value}`);
    if (url.protocol !== "https:" || url.username || url.password
        || (url.port && url.port !== "443") || url.search || url.hash
        || (url.pathname !== "/" && url.pathname !== "")
        || !/^api(?:-[a-z0-9-]+)?\.plaud\.ai$/i.test(url.hostname)) {
      throw new Error("workspace_domain_untrusted");
    }
    return url.origin;
  };
  ...
  // These rejection codes match core.client.PlaudAPIError. Other failures,
  // including rate limits and unavailable services, cannot prove expiry.
  const authRejected = (response, body) => response.status === 401 || response.status === 403
    || [-419, 419, -420, 420].includes(Number(body.status));
  ...
  try {
    ...
    const requestWorkspacePair = async () => {
      ...
      const response = await fetch(url, {
        ...
        credentials: "include", redirect: "error", signal: abortController.signal,
        body: "{}"
      });
    ...
    const refreshAccount = async () => {
      for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        if (authRejected(response, body)) throw new Error("account_session_expired");
        if (response.status !== 409 && code !== -4302) throw new Error("account_refresh_unavailable");
        await sleep(1000);
      }
      throw new Error("account_refresh_busy");
    };
    ...
    if (!businessOK(result.response, result.body)) {
      if (!authRejected(result.response, result.body)) throw new Error("workspace_token_unavailable");
    ...
    if (!businessOK(result.response, result.body)) {
      throw new Error(authRejected(result.response, result.body)
        ? "account_session_expired" : "workspace_token_unavailable");
    }
    ...
    if (!currentGeneration()) return { status: "cancelled" };
    const next = Object.assign({}, current, {
      workspaceId,
      workspaceToken: accessToken.trim(),
      expiresAt: now + Math.max(accessSeconds, 1) * 1000,
      refreshToken: refreshToken.trim(),
      refreshExpiresAt: Number.isFinite(refreshSeconds) && refreshSeconds > 0
        ? now + refreshSeconds * 1000 : null,
      domain
    });
    // Never inherit an earlier pair's expiry (including snake_case aliases).
    delete next.refresh_expires_at;
    delete next.refresh_expires_in;
    delete next.refreshExpiresIn;
    delete next.refresh_token;
    workspaceList[index] = next;
    localStorage.setItem(listKey, JSON.stringify(workspaceList));

    bridge.postMessage({
      kind: "recoveryCapture", generation: captureGeneration,
      url: result.url,
      headers: {
    ...
    const needsLogin = code === "account_session_expired";
    status(needsLogin ? "needsLogin" : "deferred", code);
    return { status: needsLogin ? "needsLogin" : "deferred", detail: code };
  }
})()
"""#
